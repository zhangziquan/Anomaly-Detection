...
def main():
    ...
    # Load models
    name = 'xray_model_04'
    net_g.load_state_dict(torch.load('./models/5rd/gen_' + name + '.pt'))
    net_d.load_state_dict(torch.load('./models/5rd/dis_' + name + '.pt'))

    print(net_g)
    print(net_d)

    # optimizer
    optimizer_g = optim.RMSprop(net_g.parameters(), lr=config.lr)
    optimizer_d = optim.RMSprop(net_d.parameters(), lr=config.lr)
    ...
    # Save models
    name = 'xray_model_05'
    torch.save(trainer.net_g.state_dict(), './models/5rd/gen_' + name + '.pt')
    torch.save(trainer.net_d.state_dict(), './models/5rd/dis_' + name + '.pt')
