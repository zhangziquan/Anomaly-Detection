...
class GoodGenerator(nn.Module):
    def __init__(self, dim=DIM, output_dim=OUTPUT_DIM):
        ...
        self.ln1 = nn.Linear(64, 4*4*8*self.dim)
...
class ConditionalDiscriminator(GoodDiscriminator):
    def __init__(self, num_classes, dim=DIM):
        super(ConditionalDiscriminator, self).__init__(dim)
        self.gamma = nn.Embedding(num_classes, 8 * self.dim)
        self.beta = nn.Embedding(num_classes, 8 * self.dim)
